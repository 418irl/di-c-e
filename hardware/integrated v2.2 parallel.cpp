...
void find_activity() {
  ...
  */
  if (gyro_readings.roll > 0.04 && gyro_readings.roll < 0.5) {
    get_activity_transition(0);
  }
  else if (gyro_readings.roll > -1.0 && gyro_readings.roll < -0.05) {
    get_activity_transition(1);
  }
  else if (gyro_readings.roll > -0.60 && gyro_readings.pitch > -1.1 ) {
    get_activity_transition(2);
  }
  else if (gyro_readings.roll > 0.7 && gyro_readings.roll < 1.3 ) {
    get_activity_transition(3);
  }
  else if (gyro_readings.roll < -40 && gyro_readings.pitch > -40 ) {
